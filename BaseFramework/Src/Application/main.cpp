...
void Application::Execute()
{
	...
	while (1)
	{
		...
		// バックバッファ　クリア時の色
		Math::Color backColor(frameRCnt * 0.01f,frameGCnt* 0.01f,frameBCnt* 0.01f, 1.0f);  // RGB A

		/*if (frameRCnt > 99 && frameBCnt < 1) { frameGCnt++; }

		if (frameGCnt > 99 && frameRCnt < 1) { frameBCnt++; }

		if (frameBCnt > 99 && frameGCnt < 1) { frameRCnt++; }*/



		//バックバッファ クリア
		D3D.WorkDevContext()->ClearRenderTargetView(
			D3D.WorkBackBuffer()->WorkRTView(), backColor);
